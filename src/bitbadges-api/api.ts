...
export async function getAddressesBySurveyId(surveyId: string, _requestBody?: GetAddressesBySurveyIdRouteRequestBody): Promise<GetAddressesBySurveyIdRouteSuccessResponse> {
  try {
    return (await BitBadgesApi.getAddressesBySurveyId(surveyId));
  } catch (error) {
    await handleApiError(error);
    return Promise.reject(error);
  }
...
export async function addAddressToSurvey(surveyId: string, requestBody: AddAddressToSurveyRouteRequestBody): Promise<AddAddressToSurveyRouteSuccessResponse> {
  try {
    return (await BitBadgesApi.addAddressToSurvey(surveyId, requestBody.address));
  } catch (error) {
    await handleApiError(error);
    return Promise.reject(error);
  }
