...
export function UpdateCollectionTimeline() {
  ...
  const items: TimelineItem[] = [(existingCollectionId && existingCollectionId > 0n) || isUpdateAddressList ? EmptyStepItem : ChooseBadgeType];
  ...
  if (mintType === MintType.BitBadge) {
    ...
    const toShowUpdateBadgeMetadataAction = canUpdateBadgeMetadataDetails.dataSource.some(
      (x) =>
        !x.forbidden &&
    ...
    const canUpdateMintCollectionApprovalsDetails = getDetailsForPermission(
      (startingCollection.collectionPermissions.canUpdateCollectionApprovals ?? []).filter((x) => x.fromList && x.fromList.checkAddress('Mint')),
      'canUpdateCollectionApprovals'
    );

    const canUpdateNonMintCollectionApprovalsDetails = getDetailsForPermission(
      (startingCollection.collectionPermissions.canUpdateCollectionApprovals ?? []).filter(
        (x) => !(x.fromList.whitelist && x.fromList.addresses.length == 1 && x.fromList.addresses[0] === 'Mint')
      ),
      'canUpdateCollectionApprovals'
    );
    ...
    if (templateCollectionSelect) {
      ...
      items.push(
        ...
        toShowUpdateStandardsAction ? StandardsSelectStep : EmptyStepItem,
        !completeControl && hasManager && toShowCanUpdateStandardsPermission ? CanUpdateStandardsSelectStep : EmptyStepItem,
        ...
        !isOffChainBalances && !isNonIndexedBalances && !isDefaultUserBalances && !noBalancesStandard ? DefaultToApprovedStepItem : EmptyStepItem,
        !isOffChainBalances && !isNonIndexedBalances && toShowUpdateMintTransfersAction && !noBalancesStandard && !isDefaultUserBalances
        ...
        isOffChainBalances && toShowUpdateOffChainBalancesMetadataAction && !noBalancesStandard ? OffChainBalancesStorageStepItem : EmptyStepItem,
        (isOffChainBalances || isNonIndexedBalances) && !noBalancesStandard && toShowCanUpdateOffChainBalancesMetadataPermission
          ? CanUpdateBytesStep
          : EmptyStepItem,
    ...
    items.push(AddressListSelectItem, SetAddressListMetadataStep, CreateAddressListStep);
  }

  return <FormTimeline formStepNum={formStepNum} setFormStepNum={setFormStepNum} items={items} />;
}
