...
export function SetBadgeMetadataStepItem() {
  ...
  return {
    ...
    node: () => (
      <UpdateSelectWrapper
        doNotUpdateNode={() => {
          return <BadgeAvatarDisplay collectionId={NEW_COLLECTION_ID} badgeIds={collection.getBadgeIdRange()} />;
        }}
        ...
        disableJson
        node={() => <div>{collection && <MetadataForm badgeIds={toUpdateBadges} addMethod={addMethod} setAddMethod={setAddMethod} />}</div>}
      />
    ),
    disabled:
      !collection ||
      (addMethod === MetadataAddMethod.Manual && collection.cachedBadgeMetadata.length == 0) ||
      (addMethod === MetadataAddMethod.UploadUrl && collection.badgeMetadataTimeline.length == 0) ||
      !!err
  };
}
