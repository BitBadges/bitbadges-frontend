...
export function DistributionMethodStepItem() {
    ...
    <div>
      {!isOffChainBalances && (
        <div className="flex-center full-width" style={{ textAlign: 'center' }}>
          <TransferabilityTab editable showDeletedGrayedOut collectionId={NEW_COLLECTION_ID} onlyShowFromMint hideHelperMessage />
        </div>
      )}
    </div>
  );
  ...
  return {
    ...
    description:
      'You previously created badges which have been sent to the Mint address. Here, you create the collection level approvals for who can transfer from the Mint address. All transfers must be approved on the collection level.',
    node: () => (
      <>
          ...
          <UpdateSelectWrapper
            ...
            customRevertFunction={() => {
              ...
              updateCollection({
                ...
                collectionApprovals: [...prevMint, ...currentNonMint]
              });
            }}
            mintOnly
