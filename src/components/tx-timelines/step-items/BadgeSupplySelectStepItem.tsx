...
export function BadgeSupplySelectStepItem() {
  ...
  const onAddStartBalances = (balance: Balance<bigint>) => {
    if (!collection) return;
    const currBadgesToCreate = deepCopyBalances(collection.defaultBalances.balances);
    const newBadgesToCreate = deepCopyBalances([...currBadgesToCreate, balance])
    const prevNumberOfBadges = startingCollection ? getTotalNumberOfBadges(startingCollection) : 0n;
    const maxBadgeIdAdded = sortUintRangesAndMergeIfNecessary(newBadgesToCreate.map(x => x.badgeIds).flat(), true).pop()?.end || 0n;
  ...
  }, { amount: 1n, badgeIds: [{ start: 1n, end: 10000n }], ownershipTimes: [{ start: 1n, end: GO_MAX_UINT_64 }] }];

  return {
    title: `Create Badges`,
    description: 
    isNonIndexed ? 'Define the number of badges to be used in your collection.' :
    ...
    node: () => <UpdateSelectWrapper
      ...
      node={() => <div className='primary-text' style={{ textAlign: 'center', justifyContent: 'center', alignItems: 'center' }}>
          {isNonIndexed && <div className="flex-center flex-column" style={{ textAlign: 'center', justifyContent: 'center', alignItems: 'center' }}>
            <BadgeIdRangesInput 
              ...
              collectionId={NEW_COLLECTION_ID}
              suggestedRanges={
                badgesToCreate.length > 0 ? badgesToCreate.map(x => x.badgeIds).flat() : deepCopyBalances(SuggestedEmptyBalances).map(x => x.badgeIds).flat()
              }
          ...
          {!isNonIndexed && <>
            ...
            {collection?.balancesType === 'Standard' && isCreateTx && <>
              ...
              <div className="secondary-text" style={{ textAlign: 'center', justifyContent: 'center', alignItems: 'center', maxWidth: 800 }}>
                <InfoCircleOutlined style={{marginRight: 4}} /> {limitedSupply ? `Set the circulating supplys for badges in your collection. Created badges will initially be sent to the Mint address.
        Every transfer requires a blockchain transaction that satisifies the approval requirements for the collection, sender, and recipient. This is the recommended option.` : `ALL addresses will be given a predefined balance of badges upon first interaction with this collection. For example, all addresses start with x1 of ID 1. Because there is no limit on addresses that can be created, note that there is potentially an infinite supply of badges.`}
                </div>
                </div>
            ...
            {((collection?.balancesType === 'Standard' && limitedSupply) || (collection?.balancesType !== 'Standard') || !isCreateTx) && <>
              <BalanceInput
                ...
                suggestedBalances={badgesToCreate.length > 0 ? badgesToCreate : deepCopyBalances(SuggestedEmptyBalances)}
              />
              <Divider />
            ...
            {((collection?.balancesType === 'Standard' && !limitedSupply) && isCreateTx) && <>
              <BalanceInput
                ...
                suggestedBalances={collection.defaultBalances.balances.length > 0 ? collection.defaultBalances.balances : deepCopyBalances(SuggestedEmptyBalances)}
              />
              <Divider />
    ...
    />,
    disabled: (!existingCollectionId && !limitedSupply &&  collection?.defaultBalances.balances.length == 0) ||
      (!existingCollectionId && limitedSupply &&  badgesToCreate?.length == 0) || !!err,
  }
}