...
export function CanUpdateBalancesStepItem() {
  ...
  if (collection.offChainClaims.length > 0) return EmptyStepItem;
  ...
  return {
            ...
            <SwitchForm
              ...
              options={[
                ...
                `}
                        ...
                        <br />
                        {noManager
                          ? 'Disabled because no manager was selected.'
                          : 'This permission is tied to the manager. Please make sure the manager is set correctly.'}
                      </>
                    ),
              ...
              onSwitchChange={(idx) => {
                handleSwitchChangeIdxOnly(idx, 'canUpdateOffChainBalancesMetadata');
              }}
            />
          </>
        )}
    ...
    disabled:
      !!err || addMethod === MetadataAddMethod.UploadUrl || collection.balancesType === 'Off-Chain - Non-Indexed'
        ? false
        : (noManager &&
            permissionDetails.hasNeutralTimes &&
            !permissionDetails.hasPermanentlyPermittedTimes &&
            !permissionDetails.hasPermanentlyForbiddenTimes) ||
          (!permissionDetails.hasNeutralTimes && !permissionDetails.hasPermanentlyForbiddenTimes)
  };
}
