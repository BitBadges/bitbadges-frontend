...
import { getPermissionVariablesFromName, iCollectionPermissionsWithDetails } from 'bitbadgesjs-sdk';
...
export function UpdateSelectWrapper({
  ...
  const { question, validateFunction } = permissionName
    ? getPermissionVariablesFromName(permissionName)
    : { question: '', validateFunction: undefined };

  const nonStandardUpdateSelect = jsonPropertyPath === 'defaultUserIncomingApprovals' || !jsonPropertyPath;
  const prevPermissions = startingCollection?.collectionPermissions[`${permissionName}` as keyof iCollectionPermissionsWithDetails<bigint>];
  const noManager = collection ? neverHasManager(collection) : true;
  const permissionDataSource = nonStandardUpdateSelect ? undefined : getDetailsForCollectionPermission(startingCollection, permissionName!); //Cast bc we only use when path is "defaultUserIncomingApprovals"
  ...
  return (
    <>
        ...
        <div style={{ alignItems: 'center' }} className="flex-center flex-wrap full-width">
          ...
          {updateFlag && (!disableUndo || customRevertFunction) && (
            <IconButton
              ...
              disabled={!customRevertFunction && JSON.stringify(startingValue) === JSON.stringify(currValue)} //TODO:
              text={'Reset'}
              onClick={() => {
            ...
            options={[
              {
                ...
                message: `This value will remain as previously set.
                  ${
                    !existingCollectionId && permissionName != 'canUpdateManager' && nonStandardUpdateSelect
                      ? ' For new collections, this means the value will be empty or unset.'
                      : ''
                  }
        ...
        <>
          <InformationDisplayCard title={question}>
            <PermissionDisplayTable
              ...
              permissionName={permissionName!} //Cast bc we only use when path is "defaultUserIncomingApprovals"
      ...
      {updateFlag && !customJson && !showAdvanced && <>{node()}</>}
      {updateFlag && !customJson && showAdvanced && <>{advancedNode?.()}</>}
    </>
  );
}
