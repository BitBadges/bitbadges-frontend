...
export function SwitchForm({
  ...
  options: Array<SwitchFormOption | undefined>;
  ...
  if (showCustomOption && filteredOptions.every((x) => !x.isSelected)) {
    ...
    filteredOptions.push({
      title: (
        <>
          Custom <WarningOutlined style={{ color: 'orange' }}></WarningOutlined>
        </>
      ),
      message:
        'A custom value has been set that does not match any of the other options. This typically happens because the value was set manually, or you went back to a previous step and changed a detail that this depends on (oftentimes, the number of badges). If this was not intentional, please select one of the other options.',
      isSelected: true
    });
  }
  ...
  return (
    <>
      <div>
        <Row
          className="flex-center flex-wrap primary-text"
          style={{
            padding: '0',
            textAlign: 'center',
            alignItems: 'normal',
        ...
        >
          {filteredOptions.map((option, index) => {
            return (
              <Col
                md={fullWidthCards ? 24 : numCardsPerRow ? 24 / numCardsPerRow : 12}
                sm={24}
                xs={24}
                key={index}
                className="flex"
              ...
              >
                <Card
                  ...
                  className={
                    option.disabled
                      ? 'primary-text tertiary-blue-bg full-width flex-center card-bg'
                      : 'primary-text card-bg full-width flex-center rounded-lg'
                  }
                  style={{
                    textAlign: 'center',
