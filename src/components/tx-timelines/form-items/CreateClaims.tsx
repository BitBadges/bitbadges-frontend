...
import { Balance, BitBadgeCollection, ClaimItemWithTrees, DistributionMethod, GO_MAX_UINT_64, MINT_ACCOUNT, TransfersExtended, UserBalance, getClaimsFromClaimItems, getTransfersFromClaimItems } from 'bitbadgesjs-utils';
...
export function CreateClaims({
  ...
  const calculateNewBalances = (newClaimItems: ClaimItemWithTrees[]) => {
    console.log("ENTER CLAIM ITEMS", newClaimItems);
    ...
    console.log("SETTING CLAIM ITEMS", newClaimItems);
          ...
          ><>
              ...
              {claimItems.length > 0 && <>
                {distributionMethod === DistributionMethod.Codes && <>
                  <Typography.Text style={{ color: PRIMARY_TEXT }}>
                          ...
                          <Text copyable={
                            {
                              ...
                              text: claimItem.password ? claimItem.password : claimItem.codes.join('\n')
                            }
                          }
                            strong style={{ color: PRIMARY_TEXT, fontSize: 16 }}
                          >
                            {claimItem.password ? 'Password: ' + claimItem.password : 'Unique Codes: ' + claimItem.codes.length}
                          </Text>
