...
export function Tabs({
  ...
  tabInfo: Array<
    | {
        ...
        subMenuTrigger?: Array<'contextMenu' | 'click' | 'hover'>;
  ...
  let tabInfoFiltered = tabInfo.filter((tab) => tab != undefined) as Array<{
    key: string;
    content: string | JSX.Element;
    disabled?: boolean;
    onClick?: () => void;
    ...
    if (tab.subMenuOverlay) {
      return (
        <Dropdown
          ...
          placement="bottom"
          overlay={tab.subMenuOverlay ? <div className="primary-text">{tab.subMenuOverlay}</div> : <></>}
      ...
      return (
        <Popover key={`${tab.key}`} className="full-width" style={{ width: widthPerTab }} content={tab.popoverContent} placement="bottom">
          {menuItem}
        </Popover>
      );
    } else {
