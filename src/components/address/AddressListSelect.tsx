...
import { BitBadgesUserInfo, convertToCosmosAddress, getChainForAddress } from 'bitbadgesjs-sdk';
...
export function BatchAddressSelect({
  ...
  async function handleAddBatchUsers() {
    setLoading(true);
    const addressesList: string[] = batchAddAddressListInput
      .split(delimiter)
      .filter((a) => a !== '')
      .map((x) => x.trim());
    ...
    const accountsToUpdate: Array<BitBadgesUserInfo<bigint>> = [];
    for (const address of addressesList) {
      if (!getAccount(address)) {
        accountsToUpdate.push(
          new BitBadgesUserInfo({
            ...BitBadgesUserInfo.BlankUserInfo(),
            address: address,
  ...
  return (
    <>
          ...
          <>
            <Tooltip
              ...
              <SwapOutlined
                disabled={disabled}
                onClick={() => {
                  setEnterMethod(enterMethod == EnterMethod.Single ? EnterMethod.Batch : EnterMethod.Single);
                }}
      ...
      {enterMethod === EnterMethod.Batch && (
        <>
          ...
          <TextArea
            ...
            disabled={disabled}
            onChange={(e) => {
              setBatchAddAddressListInput(e.target.value);
            }}
          />
          <br />
          <br />
      ...
      {users.length > 0 && !hideAddresses && (
        <div>
          <div className="flex-center">
            <AddressDisplayList  users={users} setUsers={setUsers} invalidUsers={invalidUsers} title="" />
          </div>

          {invalidUsers && Object.values(invalidUsers)?.length > 0 && (
            <div className="flex-center">
              <br />
