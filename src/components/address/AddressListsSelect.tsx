...
import { AddressList, iAddressList } from 'bitbadgesjs-sdk';
import { useState } from 'react';
import IconButton from '../display/IconButton';
import { AddressDisplayList } from './AddressDisplayList';
import { BatchAddressSelect } from './AddressListSelect';

export const autoGenerateId = (addressList: iAddressList, autoGenerateListId: boolean): AddressList => {
  if (!autoGenerateListId) return new AddressList(addressList);

  const listId = AddressList.generateReservedListId(addressList);

  // Create a new AddressList with the calculated listId
  const updatedAddressList: AddressList = new AddressList({
    ...addressList,
    listId: listId
  });
...
export function AddressListSelect({
  ...
  return (
    <div className="primary-text full-width flex-column" style={{ width: '100%', maxWidth: 500 }}>
      <div>
        <br />
        <div style={{ textAlign: 'center' }}>
          <b>
            ...
            ></Switch>{' '}
            - {isIdSelect ? 'IDs' : 'Addresses'} ({addressList.addresses.length})
          </b>
        </div>
        <div style={{ textAlign: 'center' }}>
          {showErrorOnEmpty && addressList.isEmpty() && (
            <>
              <br />
