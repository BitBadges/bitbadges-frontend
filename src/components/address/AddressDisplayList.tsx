          ...
          <div key={index} className={center ? 'flex-center' : undefined} style={{  }}>
            {trackerIdList ? (
              ...
              <AddressDisplay
                ...
                fontColor={((allowedMessage || allExcept) && user != 'All') || (!allExcept && user == 'All') ? 'red' : fontColor}
                fontSize={fontSize}
              />
            )}
            {allowedMessage && allowedMessage.length > 0 && <div style={{ color: 'red' }}>Reason: {allowedMessage}</div>}
          </div>
        );
      })}
        ...
        <div className="flex-center flex-column" style={{ marginTop: 8 }}>
          ...
          {copying && (
            <>
              <br />
              <br />
              <InfoCircleOutlined style={{ marginRight: 4 }} /> We first need to fetch the details for each user in the list (if not already fetched).
