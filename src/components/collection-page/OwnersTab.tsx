...
import { BalanceDoc, BitBadgesCollection, BitBadgesUserInfo, PaginationInfo } from 'bitbadgesjs-sdk';
import { useCallback, useEffect, useMemo, useState } from 'react';
import InfiniteScroll from 'react-infinite-scroll-component';
import { BitBadgesApi, getAccounts, getOwnersForBadge } from '../../bitbadges-api/api';
...
function BalanceInfiniteScroll({
  ...
  owners: Array<BalanceDoc<bigint>>;
  ...
  const [searchedBalanceDocs, setSearchedBalanceDocs] = useState<Array<BalanceDoc<bigint>>>([]);
  ...
  return (
    <>
        ...
        <div className="flex-wrap full-width flex" style={{ flexDirection: 'row-reverse', alignItems: 'flex-end', marginTop: 12 }}>
          <SelectWithOptions
            ...
            setValue={async () => {
              ...
              const missingAccounts: Array<BitBadgesUserInfo<bigint>> = [];
              ...
              for (let i = 0; i < accountsToFetch.length; i += 25) {
                ...
                const res = await getAccounts({
                  accountsToFetch: accountsToFetchBatch.map((x) => {
                    return { address: x };
                  })
                });
            ...
            options={[
              {
                label: loading ? (
                  ...
                  <Tooltip title="Download the list of owners and their balances. We will need to fetch all balances, if missing.">
                    <DownloadOutlined />
                  </Tooltip>
                ),
                value: 'Yes'
          ...
          />
          {
            <div style={{ marginBottom: 4, flexGrow: 1 }}>
              {' '}
              {/* Add this style to make it grow */}
        ...
        <div className="flex-center flex-wrap full-width">
          {searchedBalanceDocs.map((owner, idx) => {
            return (
              <AccountDetailsTag
                key={idx}
                addressOrUsername={owner.cosmosAddress}
                onClose={() => {
      ...
      <div className="primary-text flex-center flex-wrap full-width">
        ...
        {searchedBalanceDocs.length === 0 &&
          !pagination.hasMore &&
          owners?.filter((x) => x.cosmosAddress !== 'Mint' && x.cosmosAddress !== 'Total').length === 0 && (
            <Empty //<= 2 because of Mint and Total always being there
...
export function CollectionOwnersTab({ collectionId }: { collectionId: bigint }) {
  ...
  const fetchMore = useCallback(
    async (collection?: Readonly<BitBadgesCollection<bigint>>) => {
      if (isPreview) return;
      if (!collection) return;

      //TODO: This should probably be in a thunk action, but no other place fetches owners view so it's fine for now
      const newColl = collection.clone();
      await newColl?.fetchNextForView(BitBadgesApi, 'owners', 'owners');
  ...
  useEffect(() => {
    ...
    if (owners && owners.length > 0)
      fetchAccounts(owners?.filter((x) => x.cosmosAddress !== 'Mint' && x.cosmosAddress !== 'Total').map((x) => x.cosmosAddress));
  }, [owners]);
  ...
  return (
    <InformationDisplayCard
      ...
      <div className="primary-text flex-center flex-column">
        <BalanceDisplay hideBadges={hideBadges} collectionId={collectionId} hideMessage balances={owner.balances} />
      </div>
    </InformationDisplayCard>
  );
};

export function SpecificBadgeOwnersTab({ collectionId, badgeId }: { collectionId: bigint; badgeId: bigint }) {
  const collection = useCollection(collectionId);
  const isPreview = collection?.collectionId === NEW_COLLECTION_ID;

  const [owners, setOwners] = useState<Array<BalanceDoc<bigint>>>([]);
  ...
  const fetchMore = useCallback(
    async (bookmark?: string) => {
      ...
      if (badgeId) {
        ...
        setOwners((owners) =>
          [...owners, ...badgeOwners]
            .filter((x, idx, self) => self.findIndex((y) => y.cosmosAddress === x.cosmosAddress) === idx)
            .filter((x) => x.balances.length > 0 && x.balances.some((x) => x.amount > 0))
        );
        setPagination({
