...
const RowContentDetails = ({
  ...
  const TagsValue = <> {!disapproved &&
    <div style={{ alignItems: 'center', height: '100%', }} className='flex-center flex-wrap flex-column'>
      ...
      {approvalCriteriaHasNoAmountRestrictions(approval.approvalCriteria) && <Tag
        style={{ margin: 4, backgroundColor: '#1890ff' }}
        color='#1890ff'
        className='primary-text'
      ...
      {approval.approvalCriteria?.overridesToIncomingApprovals && <Tag
        style={{ margin: 4, backgroundColor: '#FF5733' }}
        color='#1890ff'
        className='primary-text'
      ...
      {(approval.approvalCriteria?.predeterminedBalances?.incrementedBalances.incrementBadgeIdsBy ?? 0n) > 0n || (approval.approvalCriteria?.predeterminedBalances?.incrementedBalances.incrementOwnershipTimesBy ?? 0n) > 0n && <Tag
        style={{ margin: 4, backgroundColor: '#1890ff' }}
        color='#1890ff'
        className='primary-text'
  ...
  return <tr style={{ opacity: grayedOut ? 0.5 : undefined, fontWeight: 'bold', fontSize: 16 }}>
    {
      !disapproved ?
        ...
        <td>{<span style={{ fontSize: 20, marginLeft: 4, marginRight: 24, color: 'red' }}><CloseCircleFilled /></span>} </td>}
