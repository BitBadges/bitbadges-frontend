    ...
    <Popover color='black' className="primary-text" content={<div style={{ textAlign: 'center', alignItems: 'center', display: 'flex', flexDirection: 'column', backgroundColor: 'black', color: 'white' }}>
      {
        neverHasManager ? <p>
          ...
          !hasPermittedTimes && !hasForbiddenTimes && hasNeutralTimes ? <p>
            {`This permission currently is permitted for all times and values. However, it is not frozen, meaning it could be updated to be 1) permanenlty forbidden or 2) permanently allowed.`}
          </p> :
...
export function PermissionsOverview({
  ...
  return <InformationDisplayCard title={'Permissions'} span={span}>
    <>
      <>
        {!isBadgeView && <TableRow label={"Delete collection?"} value={PermissionIcon(castActionPermissionToUniversalPermission(collection.collectionPermissions.canDeleteCollection), "", ActionPermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && <TableRow label={"Archive collection?"} value={PermissionIcon(castTimedUpdatePermissionToUniversalPermission(collection.collectionPermissions.canArchiveCollection), "archived or not", TimedUpdatePermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && <TableRow label={"Update contract address?"} value={PermissionIcon(castTimedUpdatePermissionToUniversalPermission(collection.collectionPermissions.canUpdateContractAddress), "contract address", TimedUpdatePermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && collection.balancesType === "Off-Chain" && <TableRow label={"Update off-chain balances URL?"} value={PermissionIcon(castTimedUpdatePermissionToUniversalPermission(collection.collectionPermissions.canUpdateOffChainBalancesMetadata), "off-chain balances URL", TimedUpdatePermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && <TableRow label={"Update standards?"} value={PermissionIcon(castTimedUpdatePermissionToUniversalPermission(collection.collectionPermissions.canUpdateStandards), "standards", TimedUpdatePermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && <TableRow label={"Update custom data?"} value={PermissionIcon(castTimedUpdatePermissionToUniversalPermission(collection.collectionPermissions.canUpdateCustomData), "custom data", TimedUpdatePermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && <TableRow label={"Transfer manager?"} value={PermissionIcon(castTimedUpdatePermissionToUniversalPermission(collection.collectionPermissions.canUpdateManager), "manager", TimedUpdatePermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && <TableRow label={"Update collection metadata URL?"} value={PermissionIcon(castTimedUpdatePermissionToUniversalPermission(collection.collectionPermissions.canUpdateCollectionMetadata), "collection metadata", TimedUpdatePermissionUsedFlags, neverHasManager)} labelSpan={20} valueSpan={4} />}

        {/* Can Be Badge-Specific */}
        {<TableRow label={"Update badge metadata URL?"} value={PermissionIcon(castTimedUpdateWithBadgeIdsPermissionToUniversalPermission(collection.collectionPermissions.canUpdateBadgeMetadata), "badge metadata", TimedUpdateWithBadgeIdsPermissionUsedFlags, neverHasManager, badgeId)} labelSpan={20} valueSpan={4} />}
        {collection.balancesType === "Inherited" && <TableRow label={"Update inherited balances?"} value={PermissionIcon(castTimedUpdateWithBadgeIdsPermissionToUniversalPermission(collection.collectionPermissions.canUpdateInheritedBalances), "inherited balances", TimedUpdateWithBadgeIdsPermissionUsedFlags, neverHasManager, badgeId)} labelSpan={20} valueSpan={4} />}
        {<TableRow label={"Create more badges?"} value={PermissionIcon(castBalancesActionPermissionToUniversalPermission(collection.collectionPermissions.canCreateMoreBadges), "", BalancesActionPermissionUsedFlags, neverHasManager, badgeId)} labelSpan={20} valueSpan={4} />}
        {!isBadgeView && collection.balancesType === "Standard" && <TableRow label={"Update collection-level transferability?"} value={PermissionIcon(castCollectionApprovedTransferPermissionToUniversalPermission(collection.collectionPermissions.canUpdateCollectionApprovedTransfers), "collection approved transfers", ApprovedTransferPermissionUsedFlags, neverHasManager, badgeId)} labelSpan={20} valueSpan={4} />}
      </>
    </>
