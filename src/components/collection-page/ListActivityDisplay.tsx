      ...
      <div className="flex" style={{ width: '100%' }}>
        <div className="primary-text">
          ...
          <div
            ...
            {' - '}
            {activity.addedToList
              ? `Added to ${list?.whitelist ? 'whitelist' : 'blacklist'}`
              : activity.addedToList === false
                ? `Removed from ${list?.whitelist ? 'whitelist' : 'blacklist'}`
                : activity.addedToList === undefined
                  ? 'No change to whitelist/blacklist'
                  : 'Unknown'}{' '}
...
function CollapseComponent({
  ...
  activity: Array<ListActivityDoc<DesiredNumberType>>;
  addressLists: Array<BitBadgesAddressList<bigint>>;
  onDelete?: (idx: number) => void;
  paginated?: boolean;
  hasMore: boolean;
                ...
                header={
                  <>
                    <PanelHeader
                      ...
                      unseen={!!unseenCount && unseenCount > idx}
                    />
                  </>
                }>
                {
                      ...
                      <div key={idx} className="primary-text">
                        <Row>
                          <Col span={24}>
                            ...
                            {activity.addedToList
                              ? `added to the ${list?.whitelist ? 'whitelist' : 'blacklist'}`
                              : activity.addedToList === false
                                ? `removed from the ${list?.whitelist ? 'whitelist' : 'blacklist'}`
                                : activity.addedToList === undefined
                                  ? 'No change to whitelist/blacklist'
                                  : 'Unknown'}{' '}
                            at {new Date(Number(activity.timestamp)).toLocaleDateString()} {new Date(Number(activity.timestamp)).toLocaleTimeString()}
...
export function ListActivityTab({
  ...
  const [addressLists, setAddressLists] = useState<Array<BitBadgesAddressList<bigint>>>([]);
      ...
      const accountsToFetch = [
        ...new Set(
          currActivityToDisplay
            .filter((x) => (x.addresses ?? []).length == 1)
            .map((a) => {
              return [...new Set([...(a.addresses ?? [])])].filter((a) => a !== 'Mint');
            })
            .flat()
        )
      ];

      const listsToFetch = currActivityToDisplay
        .map((a) => {
          return { listId: a.listId };
        })
        .filter((x) => x);
