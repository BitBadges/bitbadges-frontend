  ...
  return (
    <div className="flex-center flex-column">
      {addresses.length > 1 ? (
        ...
        <>
          {addresses.map((x, i) => (
            <AddressDisplay key={i} addressOrUsername={x} fontSize={17} />
          ))}
        </>
      )}
    </div>
...
function PanelHeader({
  ...
  const numBadgesTransferred = UintRangeArray.From(activity.balances.map((x) => x.badgeIds).flat()).size();
      ...
      <div className="flex" style={{ width: '100%' }}>
        <div className="primary-text">
          <div className="flex flex-wrap" style={{ display: 'flex', alignItems: 'center' }}>
            {unseen && (
              <b>
                <BellFilled className="mr-2" style={{ color: 'red' }} />
              </b>
            )}
          ...
          <div
            ...
            {' - '}
            {numBadgesTransferred.toString()} Badge{numBadgesTransferred === 1n ? '' : 's'} at{' '}
            {new Date(Number(activity.timestamp)).toLocaleDateString()} {new Date(Number(activity.timestamp)).toLocaleTimeString()}
          </div>
        </div>
        <div>
          {onDelete && (
...
function CollapseComponent({
  ...
  activity: Array<TransferActivityDoc<DesiredNumberType>>;
  onDelete?: (idx: number) => void;
  paginated?: boolean;
  hasMore: boolean;
  currPage: number;
  ...
  return (
    <>
      ...
      {activity.length > 0 && (
        <Collapse className="full-width primary-text" style={{ alignItems: 'center' }} expandIconPosition="start">
                ...
                header={
                  <>
                    <PanelHeader
                      ...
                      onDelete={onDelete}
                      idx={idx}
                      unseen={!!unseenCount && idx < unseenCount}
                    />
                  </>
                }>
                {
                      ...
                      <div key={idx} className="primary-text">
                        <Row>
                          <Col span={24}>
                            ...
                            {activity.txHash && (
                              <p>
                                <a href={EXPLORER_URL + '/BitBadges/tx/' + activity.txHash} target="_blank" rel="noopener noreferrer">
                            ...
                            )}
                            {
                              <div className="secondary-text">
                                <InfoCircleOutlined />{' '}
                                {collection?.balancesType === 'Standard'
                                  ? 'Standard transfer facilitated on the blockchain.'
                                  : "This collection uses off-chain balances. This user's balances has been updated by the host server."}
                                {activity.balances.length == 0 &&
                                  ' This user previously owned badges in this collection, but their balance has now been updated to own none.'}
                              </div>
                            }
                            <br />
                          </Col>
  ...
  useEffect(() => {
    ...
    async function getActivity() {
      ...
      const accountsToFetch = [
        ...new Set(
          currActivityToDisplay
            .map((a) => {
              return [...new Set([a.from, a.to.length > 1 ? 'Mint' : a.to[0]])].filter((a) => a !== 'Mint');
            })
            .flat()
        )
      ];
