...
import { BitBadgeCollection, ClaimItem, DistributionMethod, MAX_DATE_TIMESTAMP, MINT_ACCOUNT } from "bitbadgesjs-utils";
    ...
    >
      <div style={{ textAlign: 'center', alignItems: 'center', justifyContent: 'center' }} >
        <Row style={{ display: 'flex', justifyContent: 'center' }} >
          <h1 style={{ color: PRIMARY_TEXT }}>{'Claim #' + (claimId) + `${claim.name ? ': ' + claim.name : ''}`}</h1>
        </Row>
        {isCodeDisplay && <Row>
          <div style={{
            width: '100%'
        ...
        {showClaimDisplay ?
          <div>
            ...
            {claim.description &&
              <Row style={{ display: 'flex', justifyContent: 'center' }} >
                <div style={{ color: PRIMARY_TEXT }}>{claim.description}</div>
              </Row>}
            <br />
            <BalanceDisplay
