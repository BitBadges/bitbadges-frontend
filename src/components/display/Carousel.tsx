...
const CustomCarousel: React.FC<CustomCarouselProps> = ({ title, items, page, setPage, total, showTotalMobile, numPerPage }) => {
  ...
  const itemsToDisplay = groupedItems
    .map((items, index) => {
  ...
  return (
    <div className="primary-text">
      {!isMobile && (
        <div className="flex-between flex-wrap invisible sm:visible">
      ...
      {isMobile && (
        <>
          <div className="flex">
            {title}
            {showTotalMobile && (
              <div className="primary-text" style={{ fontSize: 20, textAlign: 'center', fontWeight: 'bolder', marginLeft: 4 }}>
