...
export function DistributionOverview({
  ...
  const isOffChainBalances =
    collection && collection.balancesType == "Off-Chain - Indexed"
      ? true
      : false
  const totalSupplyBalance =
    collection?.owners.find((x) => x.cosmosAddress === "Total")?.balances ?? []
  const mintSupplyBalance =
    collection?.owners.find((x) => x.cosmosAddress === "Mint")?.balances ?? []
  const maxBadgeId = getMaxBadgeIdForCollection(collection)

  const lastFetchedAt =
    collection.owners.find((x) => x.cosmosAddress === "Mint")?.fetchedAt ?? 0n

  const isNonIndexed =
    collection && collection.balancesType == "Off-Chain - Non-Indexed"
      ? true
      : false
  let balancesTypeInfoStr = ""
  if (collection?.balancesType === "Off-Chain - Indexed") {
    balancesTypeInfoStr =
      "Balances are stored off the blockchain and controlled via a typical server (chosen by the manager). Transferring and obtaining badges is not facilitated via the blockchain but rather via this server. Balances are indexed, meaning all owners and their balances are known. Also, a log of update activity is kept."
  } else if (collection?.balancesType === "Standard") {
    balancesTypeInfoStr =
      "Transferring and obtaining badges is all facilitated via blockchain transactions."
  } else if (collection?.balancesType === "Inherited") {
    balancesTypeInfoStr =
      "Balances of a badge are inherited from some parent badge. When you obtain or transfer the parent badge, the child badge will also be obtained or transferred."
  } else if (collection?.balancesType === "Off-Chain - Non-Indexed") {
    balancesTypeInfoStr =
      "Balances are stored off-chain and controlled via a typical server. There is no verifiable total supply, and these balances do not show up in standard search results. The only way to view balances is with the balance checker."
  }
  ...
  return (
    ...
    >
      <>
        ...
        {collection.defaultBalances.balances.length > 0 && (
          <>
            ...
            <div className="secondary-text">
              <InfoCircleOutlined /> Every user is given the default balances
              upon first interaction.
            </div>
            <br />
          </>
        )}
        {collection && !isNonIndexed && totalSupplyBalance.length > 0 && (
          <>
            <TableRow
              label={
                collection.defaultBalances.balances.length > 0
        ...
        {!isSelectStep && !isOffChainBalances && !isNonIndexed && (
          <TableRow
        ...
        {(isOffChainBalances || isNonIndexed) && !isSelectStep && (
          <TableRow
            label={"Balances URL"}
            value={
              <div>
          ...
          <TableRow
            ...
            value={
              <div>
                <>
                  {lastFetchedAt
                    ? new Date(Number(lastFetchedAt)).toLocaleString()
                    : "..."}
                </>
              </div>
            }
