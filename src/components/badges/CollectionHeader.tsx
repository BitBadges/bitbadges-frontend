...
import { BadgeMetadataDetails, Metadata } from 'bitbadgesjs-sdk';
...
const { getMetadataDetailsForBadgeId } = BadgeMetadataDetails;
...
export function CollectionHeader({
  ...
  const metadata = metadataOverride
    ? metadataOverride
    : badgeId
      ? getMetadataDetailsForBadgeId(badgeId, collection?.cachedBadgeMetadata ?? [])?.metadata
  ...
  const Title = (
    <>
      <div className="flex-center-if-mobile flex-column my-2">
        ...
        {(!hideCollectionLink || showViewLink) && <>{CollectionLink}</>}
      </div>
    </>
  );
  ...
  const About = metadata?.description && (
    <>
      <div className="primary-text" id={'description2' + badgeId + collectionId + listId}>
        <MarkdownDisplay markdown={metadata?.description ?? ''} />
      </div>

      {!metadata?.description && <div className="secondary-text">No description provided.</div>}
    </>
  );

  if (multiDisplay) {
    return (
      <div className="flex flex-wrap primary-text">
        <Col style={{ textAlign: 'start', width: '100%' }}>
          <div className="flex-center flex-column" style={{ width: '100%' }}>
            {Avatar}
            <div style={{ overflow: 'hidden', textAlign: 'center' }}>{Title}</div>
          </div>
        </Col>
      </div>
    );
        ...
        <Col style={{ textAlign: 'start', width: '100%' }}>
          <Col md={24} xs={0} sm={0} style={{ minHeight: 200, marginTop: 10 }}>
            <div style={{ width: '100%', display: 'flex' }}>
              <div style={{ flex: '0 0 300px', marginRight: '32px' }}>{Avatar}</div>
          ...
          <Col md={0} xs={24} sm={24} style={{ minHeight: 200, marginTop: 10 }}>
            <div className="flex-center flex-column">
              <div style={{ flex: '0 0 300px' }}>{Avatar}</div>
            </div>
