...
export function RefreshModal({
  ...
  return (
    <GenericModal visible={visible} setVisible={setVisible} title={'Refresh Collection'}>
      {inQueue && (
        <>
          <div className="secondary-text">{'This collection is currently in the refresh queue.'}</div>
        </>
      )}
      {!inQueue && refreshDoc && (
      ...
      {!!recentlyRefreshed && (
        <>
          <div className="secondary-text">{'This collection was recently refreshed. There is a cooldown period of 5 minutes between refreshes.'}</div>
        </>
      )}
      ...
      {errorDocs && errorDocs?.length > 0 && (
        <>
          <br />
          <div className="secondary-text">{'The following errors have been encountered:'}</div>
          <div className="secondary-text">
            {errorDocs.map((errorDoc, i) => {
              return (
                <div key={i}>
                  <br />
                  <div>
                    <b>{errorDoc._docId.includes('balance') ? 'Balances' : 'Metadata'} Error</b>
                  </div>
                  <div>
                    <b>Attempted {errorDoc.numRetries.toString()} Times</b>
                  </div>
                  <div>Next Retry Time: {new Date(Number(errorDoc.nextFetchTime)).toLocaleString()}</div>
                  <div>
                    {'Error: '}
                    <Text type="danger">{errorDoc.error}</Text>
                  </div>
                </div>
              );
            })}
