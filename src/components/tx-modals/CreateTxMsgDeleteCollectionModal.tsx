...
export function CreateTxMsgDeleteCollectionModal({
  collectionId,
  visible,
  setVisible,
  children
  ...
  const txsInfo = useMemo(() => {
    const txCosmosMsg = new MsgDeleteCollection<bigint>({
    ...
    return [
      {
        ...
        afterTx: async () => {
          notification.success({
            message: 'Collection deleted successfully! Redirecting to home page...'
          });
          await router.push('/');
        }
      }
  ...
  return (
    <TxModal msgSteps={items} visible={visible} setVisible={setVisible} txsInfo={txsInfo} txName="Delete Collection">
      {children}
    </TxModal>
  );
}
