...
import { AccountMap, BitBadgesUserInfo, AccountResponse, MINT_ACCOUNT, convertToBitBadgesUserInfo, isAddressValid } from 'bitbadgesjs-utils';
...
export const AccountsContextProvider: React.FC<Props> = ({ children }) => {
  ...
  const fetchAccounts = async (_accountsToFetch: string[], forceful?: boolean) => {

    try {
      const accountsToFetch = [...new Set(_accountsToFetch)].filter(address => address !== 'Mint' && isAddressValid(address));
  ...
  const fetchAccountsByNumber = async (_accountNums: number[], forceful?: boolean) => {
    try {
      const accountNums = [...new Set(_accountNums)].filter(accountNum => accountNum > 0);
