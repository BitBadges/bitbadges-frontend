...
import { fetchCollections, updateCollection, useCollection } from '../../bitbadges-api/contexts/collections/CollectionsContext';
...
function CollectionPage({
  ...
  const collectionIdNumber =
    collectionId && !isPreview && typeof collectionId === 'string' ? BigInt(collectionId) : isPreview ? NEW_COLLECTION_ID : -1n;
            ...
            <div
              ...
              {collection && (
                <>
                  ...
                  {/* Tab Content */}
                  {tab === 'overview' && <OverviewTab collectionId={collectionIdNumber} setTab={setTab} />}
                  {tab === 'owners' && <CollectionOwnersTab collectionId={collectionIdNumber} />}
                  {tab === 'badges' && <BadgesTab collectionId={collectionIdNumber} />}
                  ...
                  {tab === 'approvals' && !isPreview && <UserApprovalsTab collectionId={collectionIdNumber} />}
                  ...
                  {tab === 'reputation' && !isPreview && (
                    <ReputationTab
                      ...
                      fetchMore={async () => {
                        //TODO: Do as a thunk action
                        const newColl = collection.clone();
                        await newColl.fetchNextForView(BitBadgesApi, 'reviews', 'reviews');
                  ...
                  {tab === 'actions' && !isPreview && <ActionsTab collectionId={collectionIdNumber} />}

                  {tab === 'activity' && !isPreview && collection && (
                    <ActivityTab
                      activity={collection.getActivityView('transferActivity')}
                      fetchMore={async () => {
                        //TODO: Do as a thunk action
                        const newColl = collection.clone();
                  ...
                  {tab === 'history' && !isPreview && (
                    <div className="primary-text">
                      <br />
                      {collection
                        .clone()
                        .updateHistory.sort((a, b) => (Number(a.blockTimestamp) > Number(b.blockTimestamp) ? -1 : 1))
                        .map((update, i) => {
