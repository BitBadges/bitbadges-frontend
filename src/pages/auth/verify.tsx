...
const NumberInputWithSwitch = ({
  ...
  return (
    <div className="flex-center flex-column">
      <b style={{ marginBottom: 8, textAlign: 'center' }}>{title}</b>
...
function BlockinCodesScreen() {
  ...
  const { generateNonce, allowAddressSelect, storeInAccount } = router.query;
  ...
  interface QRSession {
    usedQrCodes: Array<iBlockinAuthSignatureDoc<bigint> & { usedAt: bigint }>;
  ...
  const createSession = (id: string) => {
    const newSession = {
      ...
      verificationOptions: {
        usesPerQrCode: 1,
        usesPerAddress: 1,
        addressList: AddressList.AllAddresses()
      }
    };
  ...
  useEffect(() => {
    ...
    setSessions(
      savedSessions.map((x: any) => {
        return {
          ...x,
          verificationOptions: {
            ...x.verificationOptions,
            badgeIdBounds: x.verificationOptions.badgeIdBounds?.map((x: any) => new UintRange(x).convert(BigIntify)) || []
          }
        };
      }) || []
    );
  ...
  const verify = async () => {
    try {
      ...
      const doc: BlockinAuthSignatureDoc<bigint> = new BlockinAuthSignatureDoc({
        signature: inputtedQrCode,
      ...
      if (currentSession) {
        ...
        if (
          ...
          }).checkAddress(convertToCosmosAddress(params.address))
        ) {
          setVerified(false);
        ...
        if (currentSession.verificationOptions) {
          if (currentSession.verificationOptions.usesPerQrCode && currentSession.verificationOptions.usesPerQrCode <= usesPerQrCode) {
            setVerified(false);
            setErrorMessage(
              `This QR code has been used too many times. It can only be used ${currentSession.verificationOptions.usesPerQrCode} times.`
            );
            setLoading(false);
          ...
          if (currentSession.verificationOptions.usesPerAddress && currentSession.verificationOptions.usesPerAddress <= usesPerAddress) {
            setVerified(false);
            setErrorMessage(
              `This address has been used too many times. It can only be used ${currentSession.verificationOptions.usesPerAddress} times.`
            );
            setLoading(false);
  ...
  return (
    ...
    >
      <div className="flex flex-wrap">
        <InformationDisplayCard title="Enter Authentication Details" md={12} xs={24} sm={24}>
          ...
          <div className="flex-center flex-wrap">
            ...
            <div className="secondary-text" style={{ marginLeft: 8, textAlign: 'center' }}>
              <InfoCircleOutlined />
              {autoClearMs > 0
                ? ` Code will be auto verified upon entered and cleared after ${autoClearMs / 1000} seconds.`
                : ' To verify, you manually click "Verify Code"'}
            </div>
          </div>
          <br />
          {autoClearMs <= 0 && (
                  ...
                  new BlockinAuthSignatureDoc({
                    ...
                    params: {
                      ...
                      assetOwnershipRequirements: expectedChallengeParams.assetOwnershipRequirements
                        ? convertAssetConditionGroup(expectedChallengeParams.assetOwnershipRequirements, BigIntify)
                        : undefined
                    }
                  })
                }
        ...
        <InformationDisplayCard title="Valid?" md={12} xs={24} sm={24}>
          ...
          {inputtedQrCode && !loading && (
            <>
              <div className="flex-center flex-column">
                <div>
                  {verified ? (
                    ...
                    <CloseCircleFilled style={{ color: 'red', fontSize: 100 }} />
                  ) : (
                    <></>
                  )}
                </div>
                {errorMessage && (
                  <div className="secondary-text" style={{ fontSize: 16, marginBottom: 8, color: 'red' }}>
                    {errorMessage}
                  </div>
                )}
                <br />
                {verified && (
                  <div className="secondary-text" style={{ fontSize: 16, marginBottom: 8, textAlign: 'center' }}>
                    <InfoCircleOutlined /> Blockin has verified the code, all message details are as expected, address ownership (via the signature),
                    and ownership of assets / badges. You should now implement any additional checks you require (e.g. one-time use only, verify
                    nonces, etc). We leave this up to you.
                  </div>
                )}
              </div>
              <Divider />
              {verified && currCodeDoc && (
                <>
                  <AuthCode authCode={currCodeDoc} onlyShowDetails />
                </>
              )}
            </>
          )}
      ...
      <InformationDisplayCard title="Session Manager" md={24} xs={24} sm={24}>
        <div className="flex-center">
          <div className="secondary-text" style={{ fontSize: 16, marginBottom: 8, textAlign: 'center' }}>
            <InfoCircleOutlined /> Sessions are used to track which codes have been used, which addresses have been used, etc. Sessions are stored in
            your browser, meaning their state will persist across page loads from the same device.
          </div>
        </div>
        <div className="flex-center">
          <Tabs
            tabInfo={sessions.map((x, i) => {
              return {
                key: x.id,
                content:
                  currentSession?.id === x.id ? (
                    <div className="flex-center">
                      {x.id}
                      <IconButton
                        src={
        ...
        {currentSession && (
          <div className="flex-center flex-column">
            ...
            {currentSession.usedQrCodes.map((x, i) => {
              return (
                <div key={i} style={{ margin: 8 }} className="flex">
                  ...
                  <div className="flex-center">
                    <IconButton
                      ...
                      src={<DeleteOutlined />}
                      text="Delete"
                      tooltipMessage="Delete this code from the list of used codes"
                    />
                  </div>
                </div>
              );
