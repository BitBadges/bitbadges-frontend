...
export const AuthCode = ({
  ...
  setSavedAuthCodes?: (codes: Array<BlockinAuthSignatureDoc<bigint>>) => void;
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setCurrStatus({
          success: res.verificationResponse.success,
          verificationMessage: res.verificationResponse.success ? 'Successfully verified.' : res.verificationResponse.errorMessage ?? ''
        });
      } catch (e: any) {
        console.log('ERR', e);
                ...
                {tab === 'code' && (
                  <>
                    ...
                    <QrCodeDisplay
                      ...
                      helperDisplay={
                        <div className="secondary-text" style={{ fontSize: 16, marginBottom: 8, textAlign: 'center' }}>
                          <WarningOutlined style={{ color: 'orange', marginRight: 8 }} /> Anyone with this code can authenticate as you. Keep it safe
                          and secret.
                          {!ported && (
                            <>
                              <br />
                              ...
                              {notStoredInAccount ? (
                                <>
                                  <div className="" style={{ textAlign: 'center', color: 'orange', fontSize: 16 }}>
                                    <WarningOutlined /> This code is only displayed once. It will not be stored in your BitBadges account. Please save
                                    it somewhere safe.
                                  </div>
                                </>
                              ) : (
                                <>
                                  <WarningOutlined style={{ color: 'orange', marginRight: 8 }} /> This code is stored in your BitBadges account and
                                  can be accessed under the Authentication Codes page if signed in. Note that signing in requires access to your
                                  crypto wallet. If you will not have access to your wallet at authentication time, we recommend storing the code
                                  elsewhere.
                                </>
                              )}
                            </>
          ...
          <InformationDisplayCard span={24} title={isExpected ? 'Expected Details' : 'Details'} inheritBg noBorder>
            ...
            {authCode.signature && (
              <TableRow
                label={'Code'}
                value={
                  <Tooltip title={authCode.signature}>
                    <div style={{ float: 'right' }}>{getAbbreviatedAddress(authCode.signature)}</div>
                  </Tooltip>
                }
                labelSpan={8}
                valueSpan={16}
            ...
            <TableRow
              ...
              valueSpan={16}
            />
            {authCode.params.uri !== authCode.params.domain && (
              <TableRow
                label={'URI'}
                value={
            ...
            {
              <TableRow
                label={'Authenticated Times'}
                value={
                  <>
                    {getTimeRangesElement(validFrom)}
                    {BigInt(Date.now()) < validFrom[0].start || BigInt(Date.now()) > validFrom[0].end ? (
                      <CloseCircleFilled style={{ color: 'red', marginLeft: 8 }} />
                    ) : (
                      <CheckCircleFilled style={{ color: 'green', marginLeft: 8 }} />
                    )}
                  </>
                }
                labelSpan={8}
            ...
            {authCode.params.resources && !(authCode.params.resources.length === 0) ? (
              <TableRow label={'Resources'} value={authCode.params.resources.join(', ')} labelSpan={8} valueSpan={16} />
            ) : (
              <></>
            )}
...
export function AuthCodes() {
  ...
  const [authCodes, setAuthCodes] = useState<Array<BlockinAuthSignatureDoc<bigint>>>([]);
  const [tab, setTab] = useState('all');
  const [savedAuthCodes, setSavedAuthCodes] = useState<Array<BlockinAuthSignatureDoc<bigint>>>([]);

  useEffect(() => {
    if (!signedInAccount) return;

    setAuthCodes(signedInAccount.getAuthCodesView('authCodes'));
  }, [signedInAccount]);

  const getItems = (codes: Array<BlockinAuthSignatureDoc<bigint>>) => {
    return codes.map((authCode) => {
      if (!authCode) return <></>;
      return (
        <div className="full-width" key={authCode.signature}>
            ...
            <IconButton
              ...
              onClick={async () => {
                ...
                if (confirm('Are you sure you want to delete this code?')) {
                  if (newAccountInfo.views.authCodes?.ids.includes(authCode._docId)) {
                ...
                newAccountInfo.views.authCodes = newAccountInfo.views.authCodes
                  ? {
                      ...newAccountInfo.views.authCodes,
                      ids: newAccountInfo.authCodes.map((x) => x._docId).filter((x) => x !== authCode._docId)
                    }
                  : undefined;
  ...
  useEffect(() => {
    ...
    if (savedAuthCodes) {
      setSavedAuthCodes(JSON.parse(savedAuthCodes).map((x: any) => new BlockinAuthSignatureDoc(x).convert(BigIntify)));
    }
  }, []);
  ...
  return (
    <>
      <Content className="full-area" style={{ minHeight: '80vh', padding: 8 }}>
        <br />
                ...
                <div className="flex">
                  <Tabs
                    ...
                    tabInfo={[
                      {
                        key: 'all',
                        content: 'Cloud' + (authCodes.length > 0 ? ` (${authCodes.length})` : ' (0)')
                      },
                      {
                        key: 'saved',
                        content: 'Browser' + (savedAuthCodes.length > 0 ? ` (${savedAuthCodes.length})` : ' (0)')
                      }
                    ]}
                  />
                </div>
