...
function Broadcast() {
  ...
  const parseJsonMsg = (inputMsg: string, txType: string) => {
    try {
      let msg = JSON.parse(inputMsg);
      if (txType === 'MsgUniversalUpdateCollection') msg = new MsgUniversalUpdateCollection(msg).convert(BigIntify);
      else if (txType === 'MsgDeleteCollection') msg = new MsgDeleteCollection(msg).convert(BigIntify);
      else if (txType === 'MsgCreateAddressLists') msg = new MsgCreateAddressLists(msg).convert(BigIntify);
      else if (txType === 'MsgUpdateUserApprovals') msg = new MsgUpdateUserApprovals(msg).convert(BigIntify);
      else if (txType === 'MsgTransferBadges') msg = new MsgTransferBadges(msg).convert(BigIntify);
      else if (txType === 'MsgCreateCollection') msg = new MsgCreateCollection(msg).convert(BigIntify);
      else if (txType === 'MsgUpdateCollection') msg = new MsgUpdateCollection(msg).convert(BigIntify);
      else if (txType === 'MsgCreateProtocol') msg = new MsgCreateProtocol(msg).convert(BigIntify);
      else if (txType === 'MsgUpdateProtocol') msg = new MsgUpdateProtocol(msg).convert(BigIntify);
      else if (txType === 'MsgDeleteProtocol') msg = new MsgDeleteProtocol(msg).convert(BigIntify);
      else if (txType === 'MsgSetCollectionForProtocol') msg = new MsgSetCollectionForProtocol(msg).convert(BigIntify);
      else if (txType === 'MsgUnsetCollectionForProtocol') msg = new MsgUnsetCollectionForProtocol(msg).convert(BigIntify);
      else if (txType === 'MsgInstantiateContractCompat') msg = new MsgInstantiateContractCompat(msg).convert(BigIntify);
      else if (txType === 'MsgExecuteContractCompat') msg = new MsgExecuteContractCompat(msg).convert(BigIntify);
      else if (txType === 'MsgSend')
        msg = {
      ...
      notification.error({
        message: 'Error parsing transaction',
  ...
  useEffect(() => {
    const txsInfo = routerTxsInfo ? JSON.parse(routerTxsInfo as string) : [];
    const txsInfoWithMsgs = txsInfo.map((tx: any) => {
      const msg = tx.msg;
      const type = tx.type;
      return {
        type,
        msg: parseJsonMsg(JSON.stringify(msg, null, 2), type)
      };
    });

    setTxsInfo(txsInfoWithMsgs);
    ...
    if (!notified && routerTxsInfo) {
      ...
      notification.info({
        ...
        description:
          'The transaction type and message were loaded from the URL.' +
          (hasOpener
            ? ' Upon success, the site that directed you here will be notified of the transaction hash, and you will be redirected abck to the site.'
            : '')
      });
    }
  ...
  const getJSON = (txType: string) => {
    return txType === 'MsgUniversalUpdateCollection'
      ? sampleMsgUniversalUpdateCollection
      : txType === 'MsgDeleteCollection'
        ? sampleMsgDeleteCollection
        : txType === 'MsgCreateAddressLists'
          ? sampleMsgCreateAddressLists
          : txType === 'MsgUpdateUserApprovals'
            ? sampleMsgUpdateUserApprovals
            : txType === 'MsgTransferBadges'
              ? sampleMsgTransferBadges
              : txType === 'MsgCreateCollection'
                ? sampleMsgCreateCollection
                : txType === 'MsgUpdateCollection'
                  ? sampleMsgUpdateCollection
                  : txType === 'MsgSend'
                    ? sampleMsgSend
                    : txType === 'MsgCreateProtocol'
                      ? sampleMsgCreateProtocol
                      : txType === 'MsgUpdateProtocol'
                        ? sampleMsgUpdateProtocol
                        : txType === 'MsgDeleteProtocol'
                          ? sampleMsgDeleteProtocol
                          : txType === 'MsgSetCollectionForProtocol'
                            ? sampleMsgSetCollectionForProtocol
                            : txType === 'MsgUnsetCollectionForProtocol'
                              ? sampleMsgUnsetCollectionForProtocol
                              : txType === 'MsgInstantiateContractCompat'
                                ? sampleMsgInstantiateContractCompat
                                : txType === 'MsgExecuteContractCompat'
                                  ? sampleMsgExecuteContractCompat
        ...
        <RegisteredWrapper
          ...
          node={
            <div
                          ...
                          <CollapsePanel
                            ...
                            header={
                              <div className="primary-text full-width flex-center" style={{ justifyContent: 'space-between' }}>
                                <div style={{ textAlign: 'left', fontSize: 16, fontWeight: 500 }}>{`Msg ${idx + 1}: ${txInfo.type}`}</div>
                              </div>
                            }>
                            {
                              <>
                                ...
                                {txInfo.type !== 'MsgStoreCode' && <DevMode obj={txInfo.msg} isJsonDisplay override inheritBg noBorder />}
                              </>
                            }
                          </CollapsePanel>
                        );
                ...
                <div className="flex-center flex-column">
                  ...
                  <div className="flex-center full-width" style={{ alignItems: 'normal' }}>
                    ...
                    {inputTxType !== 'MsgStoreCode' && (
                      <InformationDisplayCard noBorder inheritBg title="" md={12} xs={24} style={{ textAlign: 'center' }}>
                        <Row className="full-width flex-center flex-column">
                          <b className="primary-text">JSON</b>
                          <br />
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const msg = parseJsonMsg(inputMsg, inputTxType);
                    ...
                    {editIdx >= 0 ? 'Edit' : 'Add Msg to Transaction'}
                  </button>
                </div>
              )}
              ...
              {txsInfo.length > 0 && <TxModal visible={visible} setVisible={setVisible} txName={'Broadcast'} txsInfo={txsInfo} />}
            </div>
          }
        />
      }
